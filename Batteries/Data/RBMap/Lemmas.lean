...
theorem mem_congr [Ord α] [Std.TransCmp (α := α) compare] {t : RBNode α} (h : compare x y = .eq) :
    Mem x t ↔ Mem y t := by
...
instance (cmp) [Std.TransCmp (α := α) cmp] (t) : Decidable (@Ordered α ⟨cmp⟩ t) :=
...
theorem ordered_iff {t : RBNode α} :
    @Ordered α ⟨cmp⟩ t ↔ t.toList.Pairwise (cmpLT cmp) := by
  induction t with
  | nil => simp
...
theorem Ordered.toList_sorted {t : RBNode α} : @Ordered α ⟨cmp⟩ t → t.toList.Pairwise (cmpLT cmp) :=
  ordered_iff.1
...
theorem Ordered.min?_le {t : RBNode α} [Std.TransCmp cmp] (ht : @Ordered α ⟨cmp⟩ t)
...
theorem Ordered.le_max? {t : RBNode α} [Std.TransCmp cmp] (ht : @Ordered α ⟨cmp⟩ t)
...
theorem Mem_reverse [Ord α] [Std.OrientedCmp (α := α) compare] {t : RBNode α} :
...
theorem Ordered.memP_iff_find? [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (ht : @Ordered α ⟨cmp⟩ t) : MemP cut t ↔ ∃ x, t.find? cut = some x := by
...
theorem Ordered.unique [Std.TransCmp (α := α) cmp] (ht : @Ordered α ⟨cmp⟩ t)
...
theorem Ordered.find?_some [Std.TransCmp (α := α) cmp] [IsStrictCut cmp cut]
    (ht : @Ordered α ⟨cmp⟩ t) : t.find? cut = some x ↔ x ∈ t ∧ cut x = .eq := by
...
theorem Ordered.upperBound?_exists [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (h : @Ordered α ⟨cmp⟩ t) : (∃ x, t.upperBound? cut = some x) ↔ ∃ x ∈ t, cut x ≠ .gt := by
...
theorem Ordered.lowerBound?_exists [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (h : @Ordered α ⟨cmp⟩ t) : (∃ x, t.lowerBound? cut = some x) ↔ ∃ x ∈ t, cut x ≠ .lt := by
...
theorem Ordered.upperBound?_least_ub [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (h : @Ordered α ⟨cmp⟩ t) (hub : ∀ {x}, ub = some x → t.All (cmpLT cmp · x)) :
...
theorem Ordered.lowerBound?_greatest_lb [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (h : @Ordered α ⟨cmp⟩ t) (hlb : ∀ {x}, lb = some x → t.All (cmpLT cmp x ·)) :
...
theorem Ordered.upperBound?_least [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (ht : @Ordered α ⟨cmp⟩ t) (H : t.upperBound? cut = some x) (hy : y ∈ t)
...
theorem Ordered.lowerBound?_greatest [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (ht : @Ordered α ⟨cmp⟩ t) (H : t.lowerBound? cut none = some x) (hy : y ∈ t)
...
theorem Ordered.memP_iff_upperBound? [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (ht : @Ordered α ⟨cmp⟩ t) : t.MemP cut ↔ ∃ x, t.upperBound? cut = some x ∧ cut x = .eq := by
...
theorem Ordered.memP_iff_lowerBound? [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    (ht : @Ordered α ⟨cmp⟩ t) : t.MemP cut ↔ ∃ x, t.lowerBound? cut = some x ∧ cut x = .eq := by
...
theorem Ordered.lowerBound?_lt [Std.TransCmp (α := α) cmp] [IsStrictCut cmp cut]
    (ht : @Ordered α ⟨cmp⟩ t) (H : t.lowerBound? cut = some x) (hy : y ∈ t) :
...
theorem Ordered.insert : ∀ {path : Path α} {t : RBNode α},
    path.Ordered cmp → @RBNode.Ordered α ⟨cmp⟩ t → t.All (path.RootOrdered cmp) → path.RootOrdered cmp v →
    t.OnRoot (cmpEq cmp v) → @RBNode.Ordered α ⟨cmp⟩ (path.insert t v)
...
theorem Ordered.erase : ∀ {path : Path α} {t : RBNode α},
    path.Ordered cmp → @RBNode.Ordered α ⟨cmp⟩ t → t.All (path.RootOrdered cmp) → @RBNode.Ordered α ⟨cmp⟩ (path.erase t)
...
theorem exists_find?_insert_self [Std.TransCmp (α := α) cmp] [IsCut cmp cut]
    {t : RBNode α} (ht : Balanced t c n) (ht₂ : @Ordered α ⟨cmp⟩ t) (hv : cut v = .eq) :
...
theorem mem_insert [Std.TransCmp (α := α) cmp] {t : RBNode α}
    (ht : Balanced t c n) (ht₂ : @Ordered α ⟨cmp⟩ t) :
...
theorem mem_congr [Ord α] [Std.TransCmp (α := α) compare] {t : RBSet α compare} (h : compare x y = .eq) :
...
theorem mem_of_mem_toList [Ord α] [Std.OrientedCmp (α := α) compare] {t : RBSet α compare} (h : x ∈ toList t) :
...
theorem mem_insert_self [Ord α] [Std.OrientedCmp (α := α) compare] (v) (t : RBSet α compare) :
    v ∈ t.insert v := mem_of_mem_toList <| mem_toList_insert_self v t

theorem mem_insert_of_eq [Ord α] [Std.TransCmp (α := α) compare] (t : RBSet α compare)
    (h : compare v v' = .eq) : v' ∈ t.insert v := (mem_congr h).1 (mem_insert_self ..)
...
theorem mem_insert_of_mem_toList [Ord α] [Std.OrientedCmp (α := α) compare] (v) {t : RBSet α compare}
...
theorem mem_insert_of_mem [Ord α] [Std.TransCmp (α := α) compare] (v) {t : RBSet α compare}
    (h : v' ∈ t) : v' ∈ t.insert v :=
...
theorem mem_insert [Ord α] [Std.TransCmp (α := α) compare] {t : RBSet α compare} :
...
theorem upperBoundP?_mem [Ord α] [Std.OrientedCmp (α := α) compare] {t : RBSet α compare}
    (h : t.upperBoundP? cut = some x) : x ∈ t := mem_of_mem_toList (upperBoundP?_mem_toList h)

theorem lowerBoundP?_mem [Ord α] [Std.OrientedCmp (α := α) compare] {t : RBSet α compare}
    (h : t.lowerBoundP? cut = some x) : x ∈ t := mem_of_mem_toList (lowerBoundP?_mem_toList h)

theorem upperBound?_mem [Ord α] [Std.OrientedCmp (α := α) compare] {t : RBSet α compare}
    (h : t.upperBound? x = some y) : y ∈ t := upperBoundP?_mem h

theorem lowerBound?_mem [Ord α] [Std.OrientedCmp (α := α) compare] {t : RBSet α compare}
    (h : t.lowerBound? x = some y) : y ∈ t := lowerBoundP?_mem h

theorem upperBoundP?_exists [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] [IsCut compare cut] :
...
theorem lowerBoundP?_exists [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] [IsCut compare cut] :
...
theorem upperBound?_exists [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] :
    (∃ y, t.upperBound? x = some y) ↔ ∃ y ∈ t, compare x y ≠ .gt := upperBoundP?_exists

theorem lowerBound?_exists [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] :
...
-/
theorem upperBoundP?_least [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] [IsCut compare cut]
...
strictly greater than the cut (so there is no exact match, and nothing closer to the cut).
-/
theorem lowerBoundP?_greatest [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] [IsCut compare cut]
...
theorem lt_upperBoundP? [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] [IsStrictCut compare cut]
...
theorem lowerBoundP?_lt [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare] [IsStrictCut compare cut]
...
theorem lt_upperBound? [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare]
    (H : t.upperBound? x = some y) (hz : z ∈ t) : compare z y = .lt ↔ compare z x = .lt :=
  (lt_upperBoundP? H hz).trans Std.OrientedCmp.gt_iff_lt

theorem lowerBound?_lt [Ord α] {t : RBSet α compare} [@Std.TransCmp α compare]
